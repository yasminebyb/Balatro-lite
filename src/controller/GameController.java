...
public class GameController {
    ...
    /** Nombre de mains accordées à chaque blind. Source de vérité unique. */
    private static final int HANDS_PER_BLIND = 4;

    /** Nombre de cartes piochées en début de tour. Source de vérité unique. */
    private static final int DRAW_SIZE = 8;
     ...
     * <p>
     * Flux : pioche {@value #DRAW_SIZE} cartes → <em>phase de défausse active</em>
     * (Extension B, tant qu'il reste des défausses et que le joueur le souhaite)
     * → sélection de 5 cartes → calcul du score.
     * </p>
     */
    private void playTurn() {
        var hand = new ArrayList<>(state.getDeck().draw(DRAW_SIZE));
     ...
     * <p>
     * Pioche {@value #DRAW_SIZE} cartes dans un {@code ArrayList} mutable
     * (nécessaire pour les remplacements de la défausse active) et transmet
     * une vue immuable à la vue.
     * </p>
     */
    public void initTurn() {
        ...
        view.showGameState(state);
        currentDraw = new ArrayList<>(state.getDeck().draw(DRAW_SIZE));
     ...
     * (Extension B).
     *
     * @param indices indices (base 0) des cartes à défausser, non null,
     *                entre 1 et 5 éléments
     * @throws NullPointerException     si {@code indices} est null
     * @throws IllegalArgumentException si {@code indices} contient plus de 5 éléments
     */
    public void onDiscardComplete(List<Integer> indices) {
        ...
        if (indices.size() > 5)
            throw new IllegalArgumentException(
                    "Cannot discard more than 5 cards, got: " + indices.size());
     ...
     * @throws IndexOutOfBoundsException si un indice est hors des bornes
     *                                   de la main courante
     */
     ...
     * <p>
     * En mode console, la validation est assurée par {@code ConsoleView}.
     * En mode Zen6, {@code onSelectionComplete} est une méthode publique
     * accessible directement — la validation y est donc indispensable.
     * </p>
     *
     * @param indices  indices à valider
     * @param handSize taille de la main courante
     * @throws IllegalArgumentException  si la taille ≠ 5 ou si des doublons existent
    ...
    /**
     * Effectue la défausse active : retire les cartes aux indices donnés,
     * les envoie en défausse, tire des cartes de remplacement et décrémente
     * le compteur de défausses.
     *
     * <p>Les indices sont traités du plus grand au plus petit pour éviter le
     * décalage lors des suppressions successives sur {@code ArrayList}.</p>
     *
     ...
     *
     * <p>
     * La récompense est toujours affichée — y compris après le dernier blind —
     * <em>avant</em> d'appeler {@link GameState#nextBlind} afin que la vue
     * reflète l'état du blind qui vient d'être battu plutôt que le suivant.
     * </p>
     */
    private void handleBeatenBlind() {
        ...
        state.nextBlind(HANDS_PER_BLIND);       
    }
}