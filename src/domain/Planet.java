 ...
 * valeurs sont appliquées dans {@code GameState}.
 *
 * À chaque blind battu, le joueur reçoit une planète choisie aléatoirement via
 * {@link #random()}. Les bonus s'accumulent si la même planète est obtenue
 * plusieurs fois.
 *
 * @see HandRank
 * @see model.GameState
...
public enum Planet {
	...
	// Cache de values() pour éviter une allocation à chaque appel de random()
	private static final Planet[] VALUES = Planet.values();

	// Instance partagée -> une seule suffit pour garantir une bonne distribution
	...
	/** @return la combinaison ciblée, jamais null */
