...
public class Zen6Renderer {
    ...
    // Fonts statiques (taille et style fixes)
    /** Famille de police unique pour toute l'interface. */
    ...
    private static final Font F_PLANETS_H  = new Font(FONT, Font.BOLD,  15);
    ...
    // Cache dynamique pour txt()/txtC() — clé = style<<8 | size
    private static final java.util.Map<Integer, Font> COURIER_CACHE = new java.util.HashMap<>();

    /** Retourne (depuis le cache) une font Courier New au style et taille demandés. */
    ...
    public void renderFrame(Graphics2D g, int w, int h,
        ...
        if (inMenu) { renderMainMenu(g, w, h); return; }
    ...
    private void renderBlindPanel(Graphics2D g, int x, int y, int pw, int ph, GameState state) {
        ...
        txtC(g, "MAINS", cx, ty, 16, Font.BOLD, MUTED); ty += 22;
        g.setFont(F_HEARTS);
        ...
        txtC(g, "DÉFAUSSES", cx, ty, 16, Font.BOLD, MUTED); ty += 20;
    ...
    private void renderPlayArea(Graphics2D g, int x, int y, int pw, int ph,
        ...
        if (active == null || active.isEmpty()) {
            ...
            int cw = Zen6View.CARD_W, ch = Zen6View.CARD_H, gap = 10;
        ...
        txtC(g, "Pioche",            leftCx, pileY + PILE_H + 14, 18, Font.BOLD,  MUTED);
        txtC(g, deckCount + " / 52", leftCx, pileY + PILE_H + 28, 18, Font.PLAIN, GOLD);
        ...
        txtC(g, "Défausse",                 rightCx, pileY + PILE_H + 14, 18, Font.BOLD,  MUTED);
        txtC(g, String.valueOf(discardCount), rightCx, pileY + PILE_H + 28, 18, Font.PLAIN, GOLD);
    }
    ...
    private void renderPlanetsPanel(Graphics2D g, int x, int y, int pw, int ph, GameState state) {
        drawPanel(g, x, y, pw, ph, BORDER);
        txtC(g, "PLANÈTES", x + pw / 2, y + 17, 18, Font.BOLD, GOLD);
        g.setColor(BORDER);
        g.drawLine(x + 6, y + 24, x + pw - 6, y + 24);

        g.setFont(F_PLANETS_H);
        g.setColor(new Color(0x7a, 0x6a, 0x5a));
        g.drawString("PLANÈTE", x + 8,       y + 36);
        g.drawString("CHI",     x + pw - 68, y + 36);
        g.drawString("MLT",     x + pw - 30, y + 36);

        Planet[] planets = Planet.values();
        int rowH = Math.min(30, (ph - 44) / Math.max(1, planets.length));

        for (int i = 0; i < planets.length; i++) {
            Planet   planet  = planets[i];
            int      ry      = y + 42 + i * rowH;
            if (ry + rowH > y + ph - 4) break;

            HandRank hr      = planet.getTarget();
            int      chips   = state.getChips(hr);
            int      mult    = state.getMult(hr);
            boolean boosted  = chips > hr.getBaseChips() || mult > hr.getBaseMult();
            Color    nc      = boosted ? GREEN_BOOST : MUTED;

            if (boosted) {
                g.setColor(new Color(0x50, 0xdc, 0x78, 20));
                g.fillRoundRect(x + 4, ry, pw - 8, rowH - 2, 3, 3);
            }
            txt(g, planet.getLabel(),     x + 8,       ry + rowH - 4, 15, boosted ? Font.BOLD : Font.PLAIN, nc);
            txt(g, String.valueOf(chips),  x + pw - 68, ry + rowH - 4, 18, Font.BOLD,  BLUE_COLD);
            txt(g, "×" + mult,            x + pw - 30, ry + rowH - 4, 18, Font.BOLD,  ORANGE_FIRE);
        }
    }
    ...
    private void renderBottomBar(Graphics2D g, int x, int y, int pw, int ph,
        ...
        String instr = "Sélectionnez vos cartes  (" + selCount + " / 5)";
        ...
        drawButton(g, btnX, btnY, btnW, BTN_H, discardLabel, dBg, dBorder, dTc);
        ...
        drawButton(g, btnX + btnW + gap, btnY, btnW, BTN_H, "⚔ JOUER LA MAIN", jBg, jBorder, jTc);

        // Cartes centrées
        if (cards != null && !cards.isEmpty()) {
            int cw      = Zen6View.CARD_W;
            int ch      = Zen6View.CARD_H;
            int spacing = Zen6View.SPACING;
            int totalW  = cards.size() * spacing - (spacing - cw);
            int startX  = x + pw / 2 - totalW / 2;
            int baseY   = y + ph - ch - 10;
            ...
            for (int i = 0; i < cards.size(); i++) {
                ...
                g.fillRoundRect(cx + 4, cy + 6, cw, ch, 12, 12);
                renderCard(g, cards.get(i), cx, cy, cw, ch, isSel, isActive);
            }
        }
    }
    ...
    private void renderCard(Graphics2D g, Card card, int x, int y, int w, int h,
        ...
        if (img != null) {
            if (selected) { g.setColor(new Color(0xc8, 0xa9, 0x6e, 90)); g.fillRoundRect(x-4,y-4,w+8,h+8,CORNER+2,CORNER+2); }
            if (active)   { g.setColor(ACTIVE_FILL);                      g.fillRoundRect(x-4,y-4,w+8,h+8,CORNER+2,CORNER+2); }
            g.drawImage(img, x, y, w, h, null);
            if (selected) { g.setColor(SEL_GOLD);   g.setStroke(new BasicStroke(2.5f)); g.drawRoundRect(x,y,w,h,CORNER,CORNER); g.setStroke(new BasicStroke(1f)); }
            if (active)   { g.setColor(ACTIVE_ORG); g.setStroke(new BasicStroke(2.5f)); g.drawRoundRect(x,y,w,h,CORNER,CORNER); g.setStroke(new BasicStroke(1f)); }
        } else {
            ...
            String  sym   = getSuitSymbol(card.suit());
            g.setFont(F_CARD_RANK); g.setColor(cc);
    ...
    private void renderMainMenu(Graphics2D g, int w, int h) {
        var bg = assets.getMenuBackground();
        if (bg != null) { g.drawImage(bg, 0, 0, w, h, null); g.setColor(new Color(0,0,0,80)); g.fillRect(0,0,w,h); }
        int bw = Zen6View.BTN_W, bh = Zen6View.BTN_H, bx = w/2 - bw/2;
        drawButton(g, bx, h/2-10, bw, bh, "⚔  JOUER",   new Color(0x4a,0x1a,0x1a), new Color(0x8a,0x3a,0x3a), new Color(0xc8,0x5a,0x5a));
        drawButton(g, bx, h/2+60, bw, bh, "✕  QUITTER", PANEL_BG, BORDER, MUTED);
    }

 
    private void drawPanel(Graphics2D g, int x, int y, int w, int h, Color border) {
        g.setColor(PANEL_BG); g.fillRoundRect(x,y,w,h,CORNER,CORNER);
        g.setColor(border);   g.drawRoundRect(x,y,w,h,CORNER,CORNER);
    }

    private void drawButton(Graphics2D g, int x, int y, int w, int h,
                            String label, Color bg, Color border, Color tc) {
        g.setColor(bg);     g.fillRoundRect(x,y,w,h,CORNER,CORNER);
        g.setColor(border); g.drawRoundRect(x,y,w,h,CORNER,CORNER);
    ...
    private void drawProgressBar(Graphics2D g, int x, int y, int w, int h, float progress) {
        g.setColor(new Color(0x08,0x05,0x02)); g.fillRoundRect(x,y,w,h,h,h);
        ...
        g.setColor(new Color(0x3a,0x1a,0x0a)); g.drawRoundRect(x,y,w,h,h,h);
    }

    private void txt(Graphics2D g, String t, int x, int y, int size, int style, Color c) {
        g.setFont(courier(style, size)); g.setColor(c); g.drawString(t, x, y);
    }

    private void txtC(Graphics2D g, String t, int cx, int cy, int size, int style, Color c) {
        g.setFont(courier(style, size)); g.setColor(c);
        FontMetrics fm = g.getFontMetrics();
